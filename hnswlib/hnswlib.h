...
class AlgorithmInterface {
 public:
    ...
    virtual std::priority_queue<std::pair<dist_t, labeltype>>
        searchKnn(const void*, size_t, std::unordered_set<tableint>& current_visited_points, BaseFilterFunctor* isIdAllowed = nullptr) const = 0;
...
AlgorithmInterface<dist_t>::searchKnnCloserFirst(const void* query_data, size_t k,
    ...
    std::unordered_set<tableint> current_visited_points;


    // here searchKnn returns the result in the order of further first
    auto ret = searchKnn(query_data, k, current_visited_points, isIdAllowed);
    {
        size_t sz = ret.size();
        result.resize(sz);
        while (!ret.empty()) {
